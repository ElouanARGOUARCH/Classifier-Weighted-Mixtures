...
save_path = "C:/Users/MB811VL/Projects/chapter/Classifier-Weighted-Mixtures/experiments/VAE/"
...
plt.tight_layout()
plt.show()
fig.savefig(save_path + "MNIST_Dataset.png", dpi = fig.dpi)
...
torch.save(model, save_path + 'VAE_Gaussian_encoder_model.pt')
with torch.no_grad():
...
plt.tight_layout()
plt.show()
fig.savefig(save_path + "VAE_Gaussian_encoder_samples.png", dpi = fig.dpi)


model = CWMVAE(d_obs, d_latent,K, hidden_dims)
model.train(train_samples, epochs, batch_size, lr, verbose = True)
print(model.log_prob(test_samples, mc_samples=100).mean())
torch.save(model, save_path +'VAE_CWM_encoder_model.pt')
with torch.no_grad():
...
for i in range(16):
    ...
    ax.tick_params(axis='both', left=False, top=False, right=False, bottom=False, labelleft=False,
                   labeltop=False, labelright=False, labelbottom=False)
    ax.imshow(VAE_CWM_samples[i].reshape(28, 28))
plt.tight_layout()
plt.show()
fig.savefig(save_path + "VAE_CWM_encoder_samples.png", dpi = fig.dpi)