...
plt.show()
fig.savefig('euler_image.png', dpi = fig.dpi)
...
plt.show()
fig.savefig('euler_samples.png', dpi = fig.dpi)
...
K = 50
model = ClassifierWeightedMixture(2,K, hidden_dims= hidden_dims)
model.EM_pretraining(transformed_samples[:30000],pre_training_epochs, verbose= True)

#display pdf after EM_pretraining
fig = plt.figure(figsize =(8,12))
plt.tick_params(left = False, right = False , labelleft = False ,labelbottom = False, bottom = False)
plot_2d_function(lambda x: torch.exp(model.log_prob(logit_transform.transform(x)).squeeze(-1) + logit_transform.log_det(x)), bins = (lines, columns), range =([[0.,1.],[0.,1.]]))
plt.show()
fig.savefig('euler_GMM.png', dpi = fig.dpi)

model.train(transformed_samples,training_epochs, batch_size,lr = lr, verbose = True)
...
plot_2d_function(lambda x: torch.exp(model.log_prob(logit_transform.transform(x)).squeeze(-1) + logit_transform.log_det(x)), bins = (lines, columns), range =([[0.,1.],[0.,1.]]))
plt.show()
fig.savefig('euler_CWM.png', dpi = fig.dpi)
torch.save(model, 'euler_CWM_model.pt')
